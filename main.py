...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://locationtest2_render_example_user:lh0B1fJxmUBvvfCimUyK64t7qPDy3jcH@dpg-cuif1ran91rc73bhjsvg-a.frankfurt-postgres.render.com/locationtest2_render_example"
...
# get all users (all attributes union)
...
def get_user_profile():
    try:
        ...
        # Find the user authentication details
        ...
        relationship_data = RelationshipData.query.filter_by(user_auth_id=user.id).first()
        
        # Retrieve user image
        user_image = UserImages.query.filter_by(user_auth_id=user.id).first()
        ...
        profile = {
            'id': user.id,
            'firstname': user_data.firstname if user_data else None,
            'lastname': user_data.lastname if user_data else None,
            'email': user.email,
            'gender': user_data.gender if user_data else None,
            'hobbies': user_data.hobbies if user_data else None,
            'image_url': image_url,
